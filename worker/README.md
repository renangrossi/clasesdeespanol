# Profesor IA — backend deployment
...
with its own name, its own KV namespace, and its own Spanish-teacher
system prompt/course catalog. It does not share state with the
English, Latin, or Ancient Greek courses' Workers, and deploying one
has no effect on the others.

> **Heads up if you're following an old copy of this repo:** this
> folder previously held a `wrangler.toml` that was an unmodified copy
> of the English course's — same Worker name (`ai-teacher`) and the
> same KV namespace id. Deploying that file as-is would have
> **overwritten the live English-course Worker** and shared its rate
> limit counters. It's been fixed here (`ai-teacher-es`, placeholder
> KV id) — if you deploy from a different checkout, double check
> `wrangler.toml` says `ai-teacher-es`, not `ai-teacher`, before running
> `wrangler deploy`.
...
```
This is just the scheme+host of GitHub Pages — the same value works for every repo/project page under that account, so it's already correct for `clasesdeespanol`. Update it only if the site moves to a custom domain.
...
```
https://ai-teacher-es.<your-account-subdomain>.workers.dev
```
Note the worker is named `ai-teacher-es` (see `wrangler.toml`) specifically so it doesn't collide with the English/Latin/Greek courses' Workers if all are deployed under the same Cloudflare account. If those other Workers were deployed under the account subdomain `englishclasses` (check their own `worker/README.md` or their live endpoint URL), yours will land on that same `<account-subdomain>.workers.dev` base too, just with a different worker-name prefix — a Cloudflare account has exactly one `workers.dev` subdomain shared by every Worker in it.
...
So Profesor IA can recommend a *real* lesson link instead of guessing
one, `worker.js` imports `course-catalog.json` — a list of every real
course page (the 7 level overview pages, all 78 lesson pages, the 7
per-level "Ponte a Prueba" cumulative-review pages, and the site-wide
utility pages like Dictionary/Exercises/Placement Test) with their
real on-site URLs. At request time the Worker does a lightweight,
deterministic keyword match between the student's message (plus
recent conversation) and this catalog, and only ever hands the model
URLs that come out of that match — the model is instructed to never
output a URL that wasn't supplied to it that turn, so it's
structurally unable to invent one.

**Generated once by scraping the real `<h1>`/`href`s out of `levels/*/​*.html`, then not auto-rebuilt.** If you add, rename, or move a lesson/level page, update `course-catalog.json` by hand, or regenerate it the same way it was first built (walk `levels/<code>/*.html`, take the `<h1>` as the title and the file's relative path as `url`).

Each entry:
```json
{ "level": "A1", "title": "Adjetivos y Concordancia", "url": "levels/a1/adjetivos-y-concordancia.html", "type": "lesson" }
```
- `url` is site-root-relative (no leading slash, no domain) — the Worker resolves it against `SITE_BASE_URL` in `worker.js`.
- `type` is `"lesson"` (a level lesson page — grammar explanation AND its own exercises together), `"test"` (a level's "Ponte a Prueba" cumulative review page), or `"page"` (site-wide utility pages: Ejercicios, Diccionario, Prueba de Nivel, Exámenes Simulados, Extras, Verbos Irregulares, Repaso de Hoy, Mi Progreso).
- `level` is the CEFR level code (`"Pre-A1"`–`"C2"`), or `null` for a `"page"`-type entry that isn't level-specific.
- Optional `aliases`: extra search terms (Portuguese/English synonyms) for topics where student phrasing doesn't share words with the on-site Spanish title — none are pre-populated yet; add them by hand for topics you notice the keyword matcher missing in practice.
